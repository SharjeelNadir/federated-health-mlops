...
from .data_utils import load_node_2_data

BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))


class Node2Client(fl.client.NumPyClient):
    def __init__(self):
        X_train, y_train, X_val, y_val = load_node_2_data(BASE_DIR)
        self.X_train = X_train
        self.y_train = y_train
        self.X_val = X_val
        self.y_val = y_val

        input_dim = 8  # padded to match all nodes
    ...
    def fit(self, parameters, config):
        ...
        for epoch in range(epochs):
            ...
            print(f"[Node2] Epoch {epoch+1}/2 loss={total_loss/len(loader):.4f}")
    ...
    def evaluate(self, parameters, config):
        ...
        print(f"[Node2] Eval loss={loss:.4f}, acc={acc:.4f}")
