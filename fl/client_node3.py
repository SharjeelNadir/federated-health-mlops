...
from .data_utils import load_node_3_data

BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))


class Node3Client(fl.client.NumPyClient):
    def __init__(self):
        X_train, y_train, X_val, y_val = load_node_3_data(BASE_DIR)
    ...
    def fit(self, parameters, config):
        ...
        for epoch in range(epochs):
            ...
            print(f"[Node3] Epoch {epoch+1}/2 loss={total_loss/len(loader):.4f}")
    ...
    def evaluate(self, parameters, config):
        ...
        print(f"[Node3] Eval loss={loss:.4f}, acc={acc:.4f}")
